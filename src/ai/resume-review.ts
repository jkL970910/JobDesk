...
export function buildResumeReviewInstructions() {
  return composeSkillPrompt(skillRegistry.resumeReviewGeneral, [
    ...
    "rubric should include 4-6 items with key, label, score 0-100, maxScore 100, note, findings, evidenceQuestions, and nextAction.",
    "For each rubric item, findings must be concrete reviewer feedback for that dimension, evidenceQuestions must be questions specific to that dimension, and nextAction must tell the user what to do next. Do not put privacy/public-safe questions under project depth unless the project itself contains sensitive wording.",
