...
import {
  ...
  markEvidenceClaimsStaleForEvidenceIds,
...
export async function persistProfileEvidenceExtraction(args: {
  ...
  return getDb().transaction(async (tx) => {
    ...
    const mergedWorkExperienceDrafts = mergeWorkExperienceDrafts(
...
export async function getRecentEvidenceLibrary(limit = 8) {
  ...
  return {
    ...
    evidenceItems: evidence.map((item) => ({
      ...
      resume_eligibility: evaluateResumeEvidenceEligibility({
...
export async function markClaimsStaleForEvidenceIds(evidenceIds: string[]) {
  return markEvidenceClaimsStaleForEvidenceIds(evidenceIds);
}

export async function updateEvidenceItem(args: EvidenceAssetActionArgs) {
  return applyEvidenceAssetAction(args);
}
...
export async function deleteEvidenceItem(evidenceId: string) {
  ...
  const deleteBlocker = getEvidenceDeleteBlocker(existing);
  if (deleteBlocker) {
    return {
      status: "invalid" as const,
      reason: deleteBlocker,
    };
  }
  ...
  const result = await db.transaction(async (tx) => {
    const staleClaims =
      impactedClaimIds.length > 0
        ...
        : [];
    const taskTargetsToDelete = await tx
      .select({ id: enrichmentTaskTargets.id, taskId: enrichmentTaskTargets.taskId })
      .from(enrichmentTaskTargets)
      .where(
        and(
          eq(enrichmentTaskTargets.workspaceId, workspace.id),
    ...
    const directTasksToDelete = await tx
    ...
    const directTaskIds = new Set(directTasksToDelete.map((task) => task.id));
    const targetOnlyTaskIds = uniqueStrings(taskTargetsToDelete.map((target) => target.taskId))
      .filter((taskId) => !directTaskIds.has(taskId));
    const now = new Date();
    const targetOnlyTasks =
      targetOnlyTaskIds.length > 0
        ? await tx
            .update(enrichmentTasks)
            .set({
              dismissedAt: now,
            ...
            .where(
              and(
                eq(enrichmentTasks.workspaceId, workspace.id),
                inArray(enrichmentTasks.id, targetOnlyTaskIds),
              ),
            )
            .returning({ id: enrichmentTasks.id })
        : [];
    const taskTargets = taskTargetsToDelete.length > 0
      ? await tx
          .delete(enrichmentTaskTargets)
          .where(
            and(
              eq(enrichmentTaskTargets.workspaceId, workspace.id),
              inArray(enrichmentTaskTargets.id, taskTargetsToDelete.map((target) => target.id)),
            ),
          )
          .returning({ id: enrichmentTaskTargets.id })
      : [];
    const tasks = directTasksToDelete.length > 0
      ? await tx
          .delete(enrichmentTasks)
          .where(
            and(
              eq(enrichmentTasks.workspaceId, workspace.id),
              inArray(enrichmentTasks.id, directTasksToDelete.map((task) => task.id)),
            ),
          )
          .returning({ id: enrichmentTasks.id })
      : [];
  ...
  return {
    ...
    deletedEnrichmentTasks: result.tasks.length + result.targetOnlyTasks.length,
    deletedEnrichmentTaskTargets: result.taskTargets.length,
    dismissedTargetOnlyEnrichmentTasks: result.targetOnlyTasks.length,
    staleGeneratedClaims: result.staleClaims.length,
  };
}

type EvidenceDeleteCandidate = {
  allowedUsage: string[];
  needsUserConfirmation: number | boolean;
  publicSafeSummary: string | null;
...
function getEvidenceDeleteBlocker(item: EvidenceDeleteCandidate) {
  if (item.quarantinedAt) return "quarantined_evidence_requires_cleanup_center" as const;
  if (isResumeReadyDeleteCandidate(item)) {
    return "resume_ready_evidence_requires_quarantine" as const;
  }
  if (item.allowedUsage.includes("resume")) {
    return "resume_usage_evidence_requires_quarantine" as const;
  }
  if (item.status === "approved") return "approved_evidence_requires_quarantine" as const;
  return null;
}

function isResumeReadyDeleteCandidate(item: EvidenceDeleteCandidate) {
  const needsUserConfirmation = item.needsUserConfirmation === true || item.needsUserConfirmation === 1;
  return (
    item.status === "approved" &&
    !needsUserConfirmation &&
    item.allowedUsage.includes("resume") &&
    hasResumeSafeDisclosure({
      sensitivityLevel: item.sensitivityLevel,
      publicSafeSummary: item.publicSafeSummary,
    })
