...
|---|----------|--------|-------------------|-------|
| 1 | Resume Review: general resume versioning and scoring | Done, MVP | Passed | Supports PDF, DOCX, TXT, and Markdown upload through `/api/resume-review`; stores resume source versions, detects exact duplicate uploads by content hash, runs a staged LLM-first general resume review using the `hr-screening-review` skill adapted for no-JD scope, falls back to a local rubric when the provider is unavailable, and hands off selected versions to Evidence Library extraction or Needs Enrichment review. |
...
- OpenRouter-backed workflows can take more than one minute for longer resumes. Resume Review is now staged into smaller structured AI calls for scan, rubric scoring, and evidence/fairness review rather than weakening the prompt or analysis quality. Other long workflows still use their workflow-specific timeout and chunking policies.
