...
| Generated Resume Readiness Review | Implemented locally for the manual first loop; `generated_resume_readiness_reviews` stores generated-draft readiness reviews separately from original source Resume Review. Main Resume UI can manually run "Review generated resume" for the latest generated draft, display a scoped generated score, compare against the original source review when available, show hard gate status from Fact Guard/export policy, and let the user manually route findings to Evidence Library, Resume Builder polish, or Profile Positioning. Tailored resume readiness persistence/API support is present; Job Workspace UI wiring is a follow-up. Automatic polish proposals, accept/reject/revise, Fact Guard revalidation, rescore automation, and improvement history are not implemented yet. |
