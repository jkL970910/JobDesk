...
| Resume Review | General resume versions, scoring, strengths/weaknesses, extraction handoff | MVP implemented with staged LLM-first `hr-screening-review` skill binding, local fallback, duplicate upload detection, and extraction handoff |
...
|---|----------|--------|-------------------|-------|
| 1 | Resume Review: general resume versioning and scoring | Done, MVP | Passed | Supports PDF, DOCX, TXT, and Markdown upload through `/api/resume-review`; stores resume source versions before review AI runs, detects exact duplicate uploads by content hash, creates a persisted review run with pollable progress, runs a staged LLM-first general resume review using the `hr-screening-review` skill adapted for no-JD scope, falls back to a local rubric when the provider is unavailable, and hands off selected versions to Evidence Library extraction or Needs Enrichment review after the report is saved. |
...
| Staged Resume Review AI timeout reduction | Implemented locally; Resume Review upload now saves the source first, creates a durable `workflow_runs` review run, and publishes the report only after processing succeeds. Full Resume Review runs smaller structured AI stages for recruiter scan, rubric scoring, and evidence/fairness review, then consolidates into the existing `ResumeReview` contract. This preserves the HR-screening prompt quality while reducing single-request timeout risk. Remote QA on `https://jobdesk-tau.vercel.app` is pending after deployment |
