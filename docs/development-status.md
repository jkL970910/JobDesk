...
| Phase 7: Post-Save Validators And Work Queue Routing | In progress, second slice | Work Queue source-review pane gives scope guardrail notes a dedicated scope-review action model; Story Target rows now show deterministic review reasons and next-step suggestions for missing role, context, evidence, and public-safe wording. |
...
| S15 | Work Queue story-target creation loop | Done, MVP | Strengthen Evidence / Work Queue can create a draft Story Target when the right story is missing, then bind the current task to that new initiative or portfolio project before the user answers. Creation is only available for story/evidence/assign-later questions that can land on an experience or project; profile context, imported observations, certification/location/profile-fact notes, and source-section review notes remain excluded. The create flow shows similar existing stories first, writes `enrichment_task_targets` plus task anchor fields, and create-evidence proposals/evidence inherit the confirmed story relation without also binding role-level evidence. |
...
| Production deployment for signed-off P0 baseline | Passed; `8c9e890` deployed to Vercel production as `dpl_9mbLRh3rawgSA9aafA2HG3g6FRkq` and aliased to `https://jobdesk-tau.vercel.app`. `npm run smoke:resume-core -- --base-url https://jobdesk-tau.vercel.app` passed with QA account `jobdesk.qa.1783453313151@example.com`, Resume Review run `9a0f5d3c-f391-4bae-a5d2-7dc14e459c18`, extraction run `7d7a1d95-45f2-4b76-bf97-62aa14761fa0`, Main Resume `a895015b-7512-4228-8e91-3c98c468b3b9`, and Tailored Resume `0acfe99f-4982-49d6-87ae-6731ecb95b8e`. Main/Tailored Fact Guard executed; both drafts remained `unvalidated`, JSON audit export stayed available, and Markdown/HTML/DOCX final exports correctly returned 409. |
...
| P0 | P0.6 Full Resume Core Loop QA suite, with targeted tests added in every slice before the final E2E pass | Done, local |
